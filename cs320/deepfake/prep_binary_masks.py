...
def all_thing(input):
    dir_faceA = os.path.join(file_dir, "faceA")
    dir_faceB = os.path.join("/home/ubuntu/cs320/media", input.cleaned_data['useremail'], "faces/faceB")
    dir_bm_faceA_eyes = os.path.join("/home/ubuntu/cs320/media", input.cleaned_data['useremail'], "binary_masks/faceA_eyes")
    dir_bm_faceB_eyes = os.path.join("/home/ubuntu/cs320/media", input.cleaned_data['useremail'], "binary_masks/faceB_eyes")
    ...
    Path(dir_bm_faceB_eyes).mkdir(parents=True, exist_ok=True)
    ...
    print("Nuber of processed images in A: "+ str(num_faceA))
    print("Nuber of processed images in B: "+ str(num_faceB))
    ...
    im = cv2.resize(im, dsize=(mask.shape[0], mask.shape[1]), interpolation=cv2.INTER_LINEAR)
