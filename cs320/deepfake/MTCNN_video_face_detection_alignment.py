...
from deepfake.umeyama import umeyama
import deepfake.mtcnn_detect_face as mtcnn_detect_face
...
def all_thing(input):
    fn_input_video = os.path.join("/home/ubuntu/cs320/media", input.cleaned_data['useremail'],'_'.join(input.cleaned_data['file'].name.split(' ')))
    ...
    def create_mtcnn(sess, model_path):
        ...
        with tf.variable_scope('pnet2'):
            ...
            pnet.load(os.path.join(file_dir, "mtcnn_weights", 'det1.npy'), sess)
        with tf.variable_scope('rnet2'):
            data = tf.placeholder(tf.float32, (None,24,24,3), 'input')
            rnet = mtcnn_detect_face.RNet({'data':data})
            rnet.load(os.path.join(file_dir, "mtcnn_weights", 'det2.npy'), sess)
        with tf.variable_scope('onet2'):
            data = tf.placeholder(tf.float32, (None,48,48,3), 'input')
            onet = mtcnn_detect_face.ONet({'data':data})
            onet.load(os.path.join(file_dir, "mtcnn_weights",'det3.npy'), sess)
    ...
    Path(os.path.join("/home/ubuntu/cs320/media", input.cleaned_data['useremail'],"faces/faceB")).mkdir(parents=True, exist_ok=True)
    Path(os.path.join("/home/ubuntu/cs320/media", input.cleaned_data['useremail'],"faces/raw_faces")).mkdir(parents=True, exist_ok=True)
    Path(os.path.join("/home/ubuntu/cs320/media", input.cleaned_data['useremail'],"faces/binary_masks_eyes")).mkdir(parents=True, exist_ok=True)
    ...
    def process_video(input_img): 
        ...
        if frames % save_interval == 0:
            ...
            for idx, (x0, y1, x1, y0, conf_score) in enumerate(faces):
                ...
                fname = os.path.join("/home/ubuntu/cs320/media", input.cleaned_data['useremail'], "faces/faceB/frame{}face{}.jpg".format(frames, str(idx)))
                plt.imsave(fname, aligned_det_face_im, format="jpg")
                fname = os.path.join("/home/ubuntu/cs320/media", input.cleaned_data['useremail'], "faces/raw_faces/frame{}face{}.jpg".format(frames, str(idx)))
                ...
                fname = os.path.join("/home/ubuntu/cs320/media", input.cleaned_data['useremail'], "faces/binary_masks_eyes/frame{}face{}.jpg".format(frames, str(idx)))
    ...
    device.reset()#gpu 메모리 해제



    # ## Saved images will be in folder `faces/raw_faces` and `faces/faceB` respectively. Binary masks will be in `faces/binary_masks_eyes`.
